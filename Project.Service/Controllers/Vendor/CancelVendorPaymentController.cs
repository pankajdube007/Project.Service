...
{
    public class CancelVendorPaymentController : ApiController
    {
        [HttpPost]
        [ValidateModel]
        [Route("api/CancelVendorPayment")]
        public HttpResponseMessage GetDetails(CancelVendorPayment ula)
        {
            ...
            if (ula.transactionid != "" && ula.CIN != "")
            {
                try
                {
                    var dr = g2.return_dr("VendorTransCancel '" + ula.transactionid + "','" + ula.reasonoffailed + "'");
