...
{
    public class ProblemListAllAssignToVendorController : ApiController
    {

        [HttpPost]
        [ValidateModel]
        [Route("api/getProblemListAllAssignToVendor")]
        public HttpResponseMessage GetDetails(ListAllAssignToVendor ula)
        {
            ...
            try
            {
                ...
                List<ListVendorAll> alldcr = new List<ListVendorAll>();
                List<VendorDataAll> item = new List<VendorDataAll>();
                List<ProblemListDataAll> Problem = new List<ProblemListDataAll>();
                var dr = g1.return_dt("getItemListAllForVendordata'" + ula.VendorID + "'");
                if (dr.Rows.Count > 0)
                {
                    if (Convert.ToInt32(dr.Rows[0]["Out"]) == 1)
                    {
                        alldcr.Add(new ListVendorAll
                        {
                            result = false,
                    ...
                    else
                    {
                        ...
                        for (int i = 0; i < dr.Rows.Count; i++)
                        {
                            ...
                            var dr1 = g1.return_dt("VendorProbListAlldata '" + Convert.ToString(dr.Rows[i]["itemid"]) + "','" + Convert.ToString(dr.Rows[i]["HeadID"]) + "'");

                            // TO empty Problem Array after increment
                            Problem = new List<ProblemListDataAll>();

                            for (int j = 0; j < dr1.Rows.Count; j++)
                            {

                                Problem.Add(new ProblemListDataAll
                                {
