...
{
    public class ValidateVendorPaymentController : ApiController
    {
        [HttpPost]
        [ValidateModel]
        [Route("api/ValidateVendorPayment")]
        public HttpResponseMessage GetAllUserLatLong(ValidateVendorPaymentAction ula)
        {
            ...
            string RequestNo = cm.GetRandomKey(6);
            string otp = cm.GetRandomKey(4);
            if (ula.CIN != "")
            {
                try
                {
                    ...
                    List<ValidateVendorPayments> alldcr = new List<ValidateVendorPayments>();
                    List<ValidateVendorPayment> alldcr1 = new List<ValidateVendorPayment>();
      
                    var dr = g2.return_dr("App_VendorAddOTPUservalidation '" + ula.CIN + "','" + RequestNo + "','" + otp + "','" + ula.Category + "'");
                    if (dr.HasRows)
                    {
                        while (dr.Read())
                        {
                                ...
                                if (cm.ValidateMobile(Mobileno) == true)
                                {
                                    ...
                                    {
                                        smsbody = "Your Request No is:" + RequestNo + " and OTP is:" + otp + " OTP Validate for 5 Minutes Only. Team Goldmedal";
                                        // smsbody = "%3C%23%3E Your Request No is:" + RequestNo + " and OTP is:" + otp + " OTP Validate for 5 Minutes Only. Team Goldmedal" + ConfigurationManager.AppSettings["Gold.AutoRead.Executive"].ToString().Replace("+", "%2b");
                                    }
                                    else
                                    {
                                        smsbody = "Your Request No is:" + RequestNo + " and OTP is:" + otp + " OTP Validate for 5 Minutes Only. Team Goldmedal";
                        ...
                        // g1.close_connection();
                        g2.close_connection();
                        alldcr1.Add(new ValidateVendorPayment
                        {
                            Email = Emailid,
                            Mobile = Mobileno,
                            requestno=RequestNo,
                        ...
                        alldcr.Add(new ValidateVendorPayments
                        {
                            result = true,
