        ...
        public HttpResponseMessage GetDetails(ListsofVendorOutstanding ula)
        {
            ...
            if (ula.CIN != null)
            {
                try
                {
                    ...
                    var dr2 = g1.return_dt("IsApppaymentAvailable '" + ula.CIN + "'");
                    ...
                    if (dr2.Rows.Count > 0)
                    {
                        ...
                        else
                        {
                            g1.close_connection();
                            HttpResponseMessage response = Request.CreateResponse(HttpStatusCode.OK);
                            response.Content = new StringContent(cm.StatusTime(true, "No Oustanding available right now!!!!"), Encoding.UTF8, "application/json");
