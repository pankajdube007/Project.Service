...
{
    public class vendorsalependingorderController : ApiController
    {
        [HttpPost]
        [ValidateModel]
        [Route("api/getvendorsalependingorder")]
        public HttpResponseMessage GetDetails(Listofvendorsalependingorder ula)
        {
            ...
            if (ula.CIN != "")
            {
                try
                {
                    ...
                    var dr = g1.return_dr("Appvendorsalependingorder " + ula.vendorid + ",'" + ula.Category + "','" + ula.Date + "'," + ula.cnt + "");
                    if (dr.HasRows)
                    {
                        while (dr.Read())
                        {
