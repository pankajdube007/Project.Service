                    ...
                    if (dr.Rows.Count > 0)
                    {
                        for (int i = 0; i < dr.Rows.Count; i++)
                        {
                            alldcr1.Add(new Outstanding
                            {
                                ...
                                onlinepayment = Convert.ToBoolean(dr.Rows[i]["isonlinepaymnet"].ToString()),
                                isregistered = Convert.ToBoolean(dr.Rows[i]["freepayregister"].ToString()),
                                IsActive = Convert.ToBoolean(dr.Rows[i]["IsActive"].ToString()),
                                errormsg = Convert.ToString(dr.Rows[i]["errormsg"].ToString()),
                                duesquence = Convert.ToBoolean(dr.Rows[i]["duesquence"].ToString()),
                            });
                        }
                        g1.close_connection();
