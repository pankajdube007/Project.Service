...
using static Project.Service.Models.StatewiseSaleComparechild;

namespace Project.Service.Controllers
{
    public class StatewiseSaleComparechildController : ApiController
    {
        [HttpPost]
        [ValidateModel]
        [Route("api/getStatewiseSaleComparechild")]
        public HttpResponseMessage GetDetails(InputRequest inputRequest)
        {
            ...
            if (inputRequest.CIN != "")
            {
                try
                {
                    ...
                    List<StatewiseSaleComparechilds> alldcr1 = new List<StatewiseSaleComparechilds>();

                    var dr = g1.return_dr(string.Format("exec StatewiseSaleComparechild '{0}','{1}','{2}'", inputRequest.stateid, inputRequest.CIN, inputRequest.Category));
                    ...
                    if (dr.HasRows)
                    {
                        while (dr.Read())
                        {
                            alldcr1.Add(new StatewiseSaleComparechilds
                            {
