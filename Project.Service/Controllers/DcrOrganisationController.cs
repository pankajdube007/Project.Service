...
{
    public class DcrOrganisationController : ApiController
    {
        [HttpPost]
        [ValidateModel]
        [Route("api/GetDcrOrganationList")]
        public HttpResponseMessage GetDetails(DcrOrganisation ula)
        {
            ...
            if (ula.ExId != 0)
            {
                try
                {
                    ...
                    if (dr.HasRows)
                    {
                        while (dr.Read())
                        {
                            alldcr1.Add(new DcrOrganisationList
                            {
                        ...
                        g1.close_connection();
                        alldcr.Add(new DcrOrganisationLists
                        {
                            result = true,
