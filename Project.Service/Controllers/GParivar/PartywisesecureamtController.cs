                    ...
                    if (dr.HasRows)
                    {
                        while (dr.Read())
                        {
                            alldcr1.Add(new partysecureamt
                            {
                                ...
                                Outstanding = Convert.ToString(dr["outstng"].ToString()),
                                Secured = Convert.ToString(dr["cld"].ToString()),
                                Securedper = Convert.ToString(dr["cldper"].ToString()),
                                UnSecured = Convert.ToString(dr["balance"].ToString()),
                                UnSecuredper = Convert.ToString(dr["balanceper"].ToString()),
                                Insurance = Convert.ToString(dr["insu"].ToString()),
                            });
                        }
                        g1.close_connection();
