...
{
    public class UserstatewisecountController : ApiController
    {
        [HttpPost]
        [ValidateModel]
        [Route("api/getuserstatewisecnt")]
        public HttpResponseMessage GetDetails(ListUserstatewisecount ula)
        {
            ...
            if (ula.CIN != "")
            {
                try
                {
                    ...
                    List<UserstatewisecountLists> alldcr = new List<UserstatewisecountLists>();
                    List<UserstatewisecountList> alldcr1 = new List<UserstatewisecountList>();

                    var dr = g1.return_dr("Userstatewisecount'" + ula.CIN + "','" + ula.Cat + "','" + ula.FromDate + "','" + ula.ToDate + "','" + ula.Stateid + "'");
                    ...
                    if (dr.HasRows)
                    {
                        while (dr.Read())
                        {
                            alldcr1.Add(new UserstatewisecountList
                            {
                        ...
                        g1.close_connection();
                        alldcr.Add(new UserstatewisecountLists
                        {
                            result = true,
