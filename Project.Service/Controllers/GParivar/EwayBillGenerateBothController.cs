...
{
    public class EwayBillGenerateBothController : ApiController
    {
        [HttpPost]
        [ValidateModel]
        [Route("api/EwayBillGenerateBothPart")]
        public HttpResponseMessage GetDetails(ListsofEwayBillGenerateBoth ula)
        {
                        ...
                        if (dr3.Rows.Count > 0)
                        {
                            if (!String.IsNullOrEmpty(dr3.Rows[0]["brpin"].ToString()) && !String.IsNullOrEmpty(dr3.Rows[0]["topin"].ToString()))
                            {
                                ...
                                if (!String.IsNullOrEmpty(ula.vehicleNo) && !String.IsNullOrEmpty(ula.vehicleType) && distance != 0)
                                {
...

                                    {
                                        head.Add(new EwayBillGenerates
                                        {
                                            ...
                                            // for Production //
                                            supplyType = dr3.Rows[i]["supplyType"].ToString(),
                                            ...
                                            transDocDate = "",
                                            vehicleNo =ula.vehicleNo,
                                            vehicleType = ula.vehicleType,
                                            TransactionType = "1",
                                            itemList = child
                                      ...
                                      //  System.Net.HttpWebResponse response1 = null;
                                        ...
                                        if (response1.StatusCode == System.Net.HttpStatusCode.OK)
                                        {
                                            ...
                                            if (_output.success = true && _output.message == "E-Way Bill is generated successfully")
                                            {
                                                ...
                                                var dr6 = g2.return_dr("EwayBillupdatebyAPIBoth " + ula.slno + ",'"
                                    + _output.result.ewayBillNo + "'," + ula.userid+",'"+ula.vehicleNo+"','"+ula.drivername+"','"+ula.driverlicence+"',"+ula.logno+","+ula.type
                                ...
                                else
                                {
                                    ...
                                    response.Content = new StringContent(cm.StatusTime(false, "Oops! Atleast Vehicle No and distance, Thats missing!!!!!!!!"), Encoding.UTF8, "application/json");
