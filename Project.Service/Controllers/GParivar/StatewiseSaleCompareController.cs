...
using static Project.Service.Models.StatewiseSaleCompare;

namespace Project.Service.Controllers
{
    public class StatewiseSaleCompareController : ApiController
    {
        [HttpPost]
        [ValidateModel]
        [Route("api/GetStatewiseSalesCompare")]
        public HttpResponseMessage GetDetails(InputRequest inputRequest)
        {
            ...
            if (inputRequest.CIN != "")
            {
                try
                {
                    ...
                    List<StatewiseSaleCompares> alldcr1 = new List<StatewiseSaleCompares>();

                    var dr = g1.return_dr(string.Format("exec StatewiseSaleCompare '" +inputRequest.CIN + "','" + inputRequest.Category + "'"));
                    ...
                    if (dr.HasRows)
                    {
                        while (dr.Read())
                        {
                            alldcr1.Add(new StatewiseSaleCompares
                            {
