...
{
    public class EWayBillCancelbyInvoiceController : ApiController
    {
        [HttpPost]
        [ValidateModel]
        [Route("api/CancelEWayBillbyInvoice")]
                ...
                try
                {
                    ...
                    if (dr3.Rows.Count > 0)
                    {
                      ...
                      //    key = cm.GetEInvoiceTokanno(dr3.Rows[0]["GSTUser"].ToString(), dr3.Rows[0]["GSTPassword"].ToString(), dr3.Rows[0]["ClientID"].ToString(), dr3.Rows[0]["ClientSecret"].ToString());
                        ...
                        var baseurl = ConfigurationManager.AppSettings["Gold.Invoice.API"] + "ewaybillapi/v1.03/ewayapi";
                        ...
                        if (response1.StatusCode == System.Net.HttpStatusCode.OK)
                        {
                            ...
                            {
                                var dr1 = g2.return_dr("EwayInvoicecancelbyInvoice " + ula.slno + "," + ula.userid);
                                if (dr1.HasRows)
                                {
                                    g2.close_connection();
