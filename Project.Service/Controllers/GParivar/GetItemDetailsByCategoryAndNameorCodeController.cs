...
{
    public class GetItemDetailsByCategoryAndNameorCodeController : ApiController
    {
        ...
        [Route("api/getItemDetailsByCategoryAndNameorCode")]
        public HttpResponseMessage GetDetails(ListofGetItemDetailsByCategoryAndNameorCode ula)
        {
            ...
            if (ula.CIN != "")
            {
                try
                {
                    ...
                    List<GetItemDetailsByCategoryAndNameorCodeLists> alldcr = new List<GetItemDetailsByCategoryAndNameorCodeLists>();
                    List<GetItemDetailsByCategoryAndNameorCodeList> alldcr1 = new List<GetItemDetailsByCategoryAndNameorCodeList>();
                    var dr = g1.return_dr("GetItemDetailsByCategoryAndNameOrCode '" + ula.CIN + "','" + ula.CategoryID + "','" + ula.ItemSearchText + "'");

                    if (dr.HasRows)
                    {
                        while (dr.Read())
                        {
                            alldcr1.Add(new GetItemDetailsByCategoryAndNameorCodeList
                            {
                                ...
                                //SubCategoryId = item.rangeid,
                                //Subcategorynm = item.rangenm,
                                ...
                                //taxtype = string.IsNullOrEmpty(item.tax.ToString().Trim()) ? "" : item.tax.ToString().Trim().Split('~')[1].Split('@')[0].ToString(),
                                //taxpercent = string.IsNullOrEmpty(item.tax.ToString().Trim()) ? "" : item.tax.ToString().Trim().Split('~')[2].ToString(),
                                //pramotionaldiscount = item.promotional,
                                //ApproveQty = item.approvedqty,
                                //UnapproveQty = item.unapprovedqty,
                                //CartoonQty = item.cartonqty,
                                //BoxQty = item.boxqty,
                                //Unitnm = item.unitnm,
                        ...
                        g1.close_connection();
                        alldcr.Add(new GetItemDetailsByCategoryAndNameorCodeLists
                        {
                            result = true,
