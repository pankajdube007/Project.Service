...
{
    public class BannerImgController : ApiController
    {
        [HttpPost]
        [ValidateModel]
        [Route("api/getBannerImages")]
        public HttpResponseMessage GetDetails(BrandingImgAction ula)
        {
            ...
            if (ula.CIN != null)
            {
                try
                {
                    ...
                    var dr = g1.return_dr("App_BannerImg '" + ula.CIN + "'");
                    ...
                    if (dr.HasRows)
                    {
                        ...
                        while (dr.Read())
                        {
                            ...
                            {
                                url = string.IsNullOrEmpty(dr["item"].ToString().Trim(',')) ? "" : (baseurl + "banner/" + dr["item"].ToString().Trim(','))
                            });
                        }
