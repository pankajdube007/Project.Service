...
{
    public class MprListPendingController : ApiController
    {
        [HttpPost]
        [ValidateModel]
        [Route("api/getmprpendinglist")]
        public HttpResponseMessage GetDetails(ListMprlistpending ula)
        {
            ...
            if (ula.CIN != null)
            {
                try
                {
                    ...
                    List<Mprlistpendings> alldcr = new List<Mprlistpendings>();
                    List<Mprlistpending> alldcr1 = new List<Mprlistpending>();
                    //var dr = g1.return_dr("ManagementMPRSelectList '" + ula.CIN + "','" + ula.Category + "'");
                    var dr = g1.return_dr("hrm.ManagementMPRSelectListpending '" + ula.CIN + "','" + ula.Category + "'");
                    if (dr.HasRows)
                    {
                        while (dr.Read())
                        {
                            alldcr1.Add(new Mprlistpending
                            {
                        ...
                        g1.close_connection();
                        alldcr.Add(new Mprlistpendings
                        {
                            result = true,
