...
{
    public class StockAvailibilityExcutiveController : ApiController
    {
        [HttpPost]
        [ValidateModel]
        [Route("api/getStockAvailibilityExcutive")]
        public HttpResponseMessage GetAllUserLatLong(ListofStockAvailibilityEx ula)
        {
            ...
            if (ula.ExId != 0)
            {
                ...
                {
                    string data1;
                    List<StockAvailibilityExs> alldcr = new List<StockAvailibilityExs>();
                    List<StockAvailibilityEx> alldcr1 = new List<StockAvailibilityEx>();
                    var dr = g1.return_dr("AppExcutiveStockAvailibility " + ula.ExId + "," + ula.Qty + "," + ula.ItemId);
                    if (dr.HasRows)
                    {
                        while (dr.Read())
                        {
                        ...
                        g1.close_connection();
                        alldcr.Add(new StockAvailibilityExs
                        {
                            result = "True",
