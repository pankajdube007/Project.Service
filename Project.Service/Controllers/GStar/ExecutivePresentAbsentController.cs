...
{
    public class ExecutivePresentAbsentController : ApiController
    {
        [HttpPost]
        [ValidateModel]
        [Route("api/GetExecutivePresentAbsentList")]
        public HttpResponseMessage GetDetails(ExecutivePresentAbsent ula)
        {
            ...
            if (ula.ExId != 0)
            {
                try
                {
                    ...
                    List<ExecutivePresentAbsentLists> alldcr = new List<ExecutivePresentAbsentLists>();
                    List<ExecutivePresentAbsentList> alldcr1 = new List<ExecutivePresentAbsentList>();

                    var dr = g1.return_dr("ExecutivePresentAbsent " + ula.ExId + "");

                    if (dr.HasRows)
                    {
                        while (dr.Read())
                        {
                            alldcr1.Add(new ExecutivePresentAbsentList
                            {
                        ...
                        g1.close_connection();
                        alldcr.Add(new ExecutivePresentAbsentLists
                        {
                            result = true,
