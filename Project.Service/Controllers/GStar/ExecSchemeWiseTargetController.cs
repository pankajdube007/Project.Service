...
{
    public class ExecSchemeWiseTargetController : ApiController
    {
        [HttpPost]
        [ValidateModel]
        [Route("api/getExecSchemeWiseTarget")]
        public HttpResponseMessage GetDetails(ListExecSchemeWiseTarget ula)
        {
            ...
            if (ula.ExId != 0)
            {
                try
                {
                    ...
                    List<ExecSchemeWiseTarget> alldcr = new List<ExecSchemeWiseTarget>();
                    List<ExecSchemeWiseTargets> alldcr1 = new List<ExecSchemeWiseTargets>();
                    var dr = g1.return_dr("GCSExecSchemeWiseTarget '" + ula.ExId + "'");
                    if (dr.HasRows)
                    {
                        string baseurl = _goldMedia.MapPathToPublicUrl("");
                        while (dr.Read())
                        {
                            alldcr1.Add(new ExecSchemeWiseTargets
                            {
                        ...
                        g1.close_connection();
                        alldcr.Add(new ExecSchemeWiseTarget
                        {
                            result = true,
