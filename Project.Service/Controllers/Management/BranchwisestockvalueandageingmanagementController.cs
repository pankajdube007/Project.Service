...
{
    public class BranchwisestockvalueandageingmanagementController : ApiController
    {
        [HttpPost]
        [ValidateModel]
        [Route("api/Branchwisestockvalueandageingmanagement")]
        public HttpResponseMessage GetDetails(Branchwisestockvalueandageingmanagement ula)
        {
            ...
            if (ula.CIN != "")
            {
                try
                {
                    ...
                    List<BranchwisestockvalueandageingmanagementLists> alldcr = new List<BranchwisestockvalueandageingmanagementLists>();
                    List<BranchwisestockvalueandageingmanagementList> alldcr1 = new List<BranchwisestockvalueandageingmanagementList>();

                    var dr = g1.return_dr("Branchwisestockvalueandageingmanagement '"+ ula.CIN + "','" + ula.Category + "'");

                    if (dr.HasRows)
                    {
                        while (dr.Read())
                        {
                            alldcr1.Add(new BranchwisestockvalueandageingmanagementList
                            {
                        ...
                        g1.close_connection();
                        alldcr.Add(new BranchwisestockvalueandageingmanagementLists
                        {
                            result = true,
