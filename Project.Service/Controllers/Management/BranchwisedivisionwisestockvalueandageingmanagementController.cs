...
{
    public class BranchwisedivisionwisestockvalueandageingmanagementController : ApiController
    {
        [HttpPost]
        [ValidateModel]
        [Route("api/Branchwisedivisionwisestockvalueandageingmanagement")]
        public HttpResponseMessage GetDetails(Branchwisedivisionwisestockvalueandageingmanagement ula)
        {
            ...
            if (ula.CIN != "")
            {
                try
                {
                    ...
                    List<BranchwisedivisionwisestockvalueandageingmanagementLists> alldcr = new List<BranchwisedivisionwisestockvalueandageingmanagementLists>();
                    List<BranchwisedivisionwisestockvalueandageingmanagementList> alldcr1 = new List<BranchwisedivisionwisestockvalueandageingmanagementList>();

                    var dr = g1.return_dr("Branchwisedivisionwisestockvalueandageingmanagement '" + ula.CIN + "','" + ula.Category +"',"+ula.BranchID);

                    if (dr.HasRows)
                    {
                        while (dr.Read())
                        {
                            alldcr1.Add(new BranchwisedivisionwisestockvalueandageingmanagementList
                            {
                        ...
                        g1.close_connection();
                        alldcr.Add(new BranchwisedivisionwisestockvalueandageingmanagementLists
                        {
                            result = true,
