                ...
                try
                {
                    ...
                    if (dr.HasRows)
                    {
                        while (dr.Read())
                        {
                            ...
                            link = Convert.ToString(dr["SlNo"])=="0" ? "" : WebConfigurationManager.AppSettings["ErpUrl"].ToString() + "/Party-Ledger-Report3.aspx?partyid=" + Convert.ToString(dr["SlNo"]) + "&fromdate=" + dateformat(ula.fromdate) + "&todate=" + dateformat(ula.todate),
                            });
                        }
                        g1.close_connection();
