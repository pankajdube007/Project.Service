...
{
    public class OrderItemDetailsController : ApiController
    {
        [HttpPost]
        [ValidateModel]
        [Route("api/getOrderItemDetails")]
        public HttpResponseMessage GetDetails(OrderItemDetailsAction ula)
        {
            ...
            if (ula.CIN != null)
            {
                try
                {
                    ...
                    List<OrderItemDetailss> alldcr = new List<OrderItemDetailss>();
                    List<OrderItemDetails> alldcr1 = new List<OrderItemDetails>();
                    //var dr = g1.return_dr("AppItemPriceDetails '" + ula.CIN + "',"
                    //    +ula.CategoryId?.ToString()?? "0"+","
                    //    + ula.DivisionId?.ToString() ?? "0" + ",'"
                    //    +ula.ItemId+"'");
                    string catid = string.IsNullOrEmpty(ula.CategoryId.ToString()) ? "0" : ula.CategoryId.ToString();
                    string divid = string.IsNullOrEmpty(ula.DivisionId.ToString()) ? "0" : ula.DivisionId.ToString();
                    var dr = g1.return_dr("AppItemPriceDetails '" + ula.CIN + "',"
                        + catid + ","
                        + divid + ",'"
                        + ula.ItemId + "'");
                    ...
                    if (dr.HasRows)
                    {
                        ...
                        while (dr.Read())
                        {
                            alldcr1.Add(new OrderItemDetails
                            {
                        ...
                        g1.close_connection();
                        alldcr.Add(new OrderItemDetailss
                        {
                            result = true,
