...
{
    public class MprListController : ApiController
    {
        [HttpPost]
        [ValidateModel]
        [Route("api/getmprlist")]
        public HttpResponseMessage GetDetails(ListMprList ula)
        {
            ...
            if (ula.CIN != null)
            {
                try
                {
                    ...
                    List<MprLists> alldcr = new List<MprLists>();
                    List<MprList> alldcr1 = new List<MprList>();
                    //var dr = g1.return_dr("ManagementMPRSelectList '" + ula.CIN + "','" + ula.Category + "'");
                    var dr = g1.return_dr("hrm.ManagementMPRSelectList '" + ula.CIN + "','" + ula.Category + "'");
                    if (dr.HasRows)
                    {
                        while (dr.Read())
                        {
                            alldcr1.Add(new MprList
                            {
