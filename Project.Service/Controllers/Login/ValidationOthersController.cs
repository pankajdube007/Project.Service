...
namespace Project.Service.Controllers
{
    ...
    public class ValidationOthersController : ApiController
    {
        [HttpPost]
        [ValidateModel]
        // [HMACAuthentication]
        [Route("api/ValidateOtherUser")]
        public HttpResponseMessage ValidationOther(ValidationOtherAction ar)
        {
            ...
            if (ModelState.IsValid)
            {
                try
                {
                    ...
                    var dt = g2.return_dt("exec validuserother '" + ar.usernm + "','" + ar.pwd + "','" + ar.application + "'");
                    if (dt.Rows.Count > 0)
                    {
                        string data1;
