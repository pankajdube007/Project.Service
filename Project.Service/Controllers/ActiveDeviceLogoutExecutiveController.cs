...
{
    public class ActiveDeviceLogoutExecutiveController : ApiController
    {
        [HttpPost]
        [ValidateModel]
        [Route("api/ActiveDeviceLogoutExecutive")]
        public HttpResponseMessage GetAllUserdetails(ListofActiveDeviceLogoutExecutive ula)
        {
            ...
            if (ula.CIN != "")
            {
                try
                {
                    ...
                    List<ActiveDeviceLogoutExecutives> alldcr = new List<ActiveDeviceLogoutExecutives>();
                    List<ActiveDeviceLogoutExecutive> alldcr1 = new List<ActiveDeviceLogoutExecutive>();
                    var newpwd = cm.GenerateRandomString(10);
                    var dr = g1.return_dt("CurrentActiveDeviceLogoutExecutive '" + ula.CIN + "','" + newpwd + "'");
