...
{
    public class DesignationwiseemployeedetailController : ApiController
    {
        [HttpPost]
        [ValidateModel]
        [Route("api/getdesignationwiseempdetail")]
        public HttpResponseMessage GetDetails(ListDesignationwiseempdetail ula)
        {
            ...
            if (ula.CIN != "")
            {
                try
                {
                    ...
                    List<DesignationwiseempdetailLists> alldcr = new List<DesignationwiseempdetailLists>();
                    List<DesignationwiseempdetailList> alldcr1 = new List<DesignationwiseempdetailList>();

                    var dr = g1.return_dr("[hrm].[DesignationWiseEmployeeData] '" + ula.CIN + "','" + ula.Cat + "','" + ula.DesigId + "'");

                    if (dr.HasRows)
                    {
                        while (dr.Read())
                        {
                            alldcr1.Add(new DesignationwiseempdetailList
                            {
                        ...
                        g1.close_connection();
                        alldcr.Add(new DesignationwiseempdetailLists
                        {
                            result = true,
